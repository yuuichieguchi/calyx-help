...
herdr の行は表示専用です。
クリックしても何も起きません。
そのワークスペースを開く、またはフォーカスするには、[Session Browser](/ja/usage/sessions/#herdr-ワークスペース)を使ってください。
...
この機能を有効にすると、Claude Code と Codex はツールを実行する前に許可要求を Calyx へ送り、コックピットツールと同じバナーで一括して判断できます。
...
障害時に自動許可されることはありません。
約 10 分以内に応答しなかった場合や Calyx に接続できない場合、エージェントは自身のペイン内プロンプトに切り戻ります。
エージェント側でツール実行をキャンセルすると、対応するバナーは即座に消えます。
対象は Claude Code と Codex です。それ以外のエージェントは従来どおりペイン内で確認を求めます。
