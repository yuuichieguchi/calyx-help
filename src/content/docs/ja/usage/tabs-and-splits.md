...
グループはタブをまとめる入れ物です。
グループには作成時に 10 種類の色プリセットのどれかが割り当てられ、サイドバー上ではシェブロンで折りたたんだり展開したりできます。
...
サイドバーのグループ見出しを右クリック（または Ctrl+クリック）すると、コンテキストメニューが開きます。

- **Close Group**: グループ内のタブをすべて閉じ、グループを削除します。
- **Close Other Groups**: ほかのグループをすべて閉じます。グループが一つしかないときは選べません。
- **Close Groups Below**: このグループより下のグループをすべて閉じます。最後のグループでは選べません。
- **Rename Group...**: ダブルクリックと同じ名前の編集欄を開きます。
- **Color**: グループの色を 10 種類のプリセットから選びます。現在の色にはチェックが付きます。

タブのメニューと同じく、どの項目も右クリックしたグループを対象にし、メニューを開いてもそのグループには切り替わりません。

グループを閉じるときに確認が出るのは、グループ内の差分タブに未送信のレビューコメントがある場合だけです。
これはコンテキストメニューや `Ctrl+Shift+W` を含め、グループを閉じるすべての操作に当てはまります。
キャンセルした差分タブは開いたまま残り、グループもそのタブとともに残ります。
グループ内のほかのタブは閉じられます。
