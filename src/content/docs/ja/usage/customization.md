...
ブラウザタブの上部に出るツールバーも、macOS 標準のマテリアルではなくテーマカラーに追従します。
...
タイトルバー、タブバー、サイドバー、ターミナルが同じ面を共有していて、その境界に継ぎ目や細い線は引かれません。

システム設定のアクセシビリティにあるディスプレイで「透明度を下げる」をオンにすると、タイトルバーを含めてこの面全体が不透明になり、背景のグラデーションも描かれなくなります。
このとき、サイドバーと隣のペインのあいだにも線は引かれず、二つの面が地続きになります。
