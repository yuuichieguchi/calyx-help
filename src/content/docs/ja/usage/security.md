...
メニューでオフにしていても、フォーカス中のペインがパスワード入力待ち（`sudo`、`ssh` のパスワード入力、`read -s` など）になっているあいだは、Calyx が自動で Secure Keyboard Entry を有効にします。
このときペインの右上に鍵のアイコンが表示され、クリックすると説明が出ます。
ローカルの[永続セッション](/ja/usage/sessions/)のペインでも同じように動きます。
セッションデーモンがパスワード入力待ちを検知して Calyx に知らせるためです。
リモートの永続セッションは対象外です（[既知の制限](/ja/reference/known-limitations/)を参照）。
