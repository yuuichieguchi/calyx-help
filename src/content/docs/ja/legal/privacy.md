...
- AI エージェント連携用の設定（各エージェント自身の設定ディレクトリに書き込まれます。`~/.claude.json`、`~/.codex/`、`~/.config/opencode/`、`~/.hermes/`、`~/.grok/`。設定ファイルを持たない pi については、pi が読み込んで実行する TypeScript の拡張 `~/.pi/agent/extensions/calyx.ts`）
